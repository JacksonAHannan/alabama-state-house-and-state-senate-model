...
Alabama FCPA expenditure extracts (`data/raw/finance/alabama/*.zip`),
...
`project_docs/model/MODEL_READINESS.md`.
...
same way `data/raw/finance/alabama/` and the OpenElections CSVs are
...
- Its outputs: `data/processed/war/finance_candidate_cycle_totals.csv`,
  ...
  coverage/QA output).
- `data/raw/finance/alabama/` raw FCPA extracts become unused by this
