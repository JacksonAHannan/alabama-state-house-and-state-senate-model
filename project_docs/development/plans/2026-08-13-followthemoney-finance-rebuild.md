...
**Modify:**
- `project_docs/model/MODEL_READINESS.md` — "Rebuild and validate" script list (swap
...
as part of the regular rebuild pipeline (see project_docs/model/MODEL_READINESS.md). Requires
...
`git show HEAD:project_docs/model/MODEL_READINESS.md` or rerun on the pre-change data if
...
ballpark as `project_docs/model/MODEL_READINESS.md`'s current documented numbers — a
...
- [ ] **Step 2: Update `project_docs/model/MODEL_READINESS.md`'s "Rebuild and validate" section**
...
- [ ] **Step 3: Update `project_docs/model/MODEL_READINESS.md`'s finance-coverage figure**
...
```bash
git add -A data/processed/war project_docs/model/MODEL_READINESS.md
git status
```
Review the output: it should show Task 3's regenerated finance/WAR files as
modified, this task's deletions, `project_docs/model/MODEL_READINESS.md` as modified, and
