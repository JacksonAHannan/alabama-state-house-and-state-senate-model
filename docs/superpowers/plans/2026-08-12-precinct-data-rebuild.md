...
- `.gitignore` — add `.venv/`.
- `MODEL_READINESS.md` — update the "Rebuild and validate" script list.
...
ballpark as `MODEL_READINESS.md`'s current numbers (R-squared ~0.2,
...
- [ ] **Step 4: Update `MODEL_READINESS.md`'s "Rebuild and validate" section**
...
```bash
git add -A data/processed data/derived data/manual MODEL_READINESS.md
git status
```
...
- [ ] **Step 7: Update `MODEL_READINESS.md`'s "Current result" section**
...
```bash
git add MODEL_READINESS.md
git commit -m "Update MODEL_READINESS.md with post-rebuild validation numbers"
```
