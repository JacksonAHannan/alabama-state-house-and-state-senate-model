...
Precinct shapefiles primarily sourced from VEST's 2020 General Election Shapefile, additional precinct information was retrieved in the following counties: Autaga, Baldwin, Bullock, Butler, Calhoun, Coffee, Colbert, Henry, Jefferson, Madison, Marshall, Mobile, Montgomery, Shelby and Walker.

Modifications were made to the precinct shapefiles using Alabama voter file, via L2 dated 1/18/23.
...
Precinct boundaries were constructed by first attempting to link 2022 general election precincts to the 2020 general election precincts by name. In instances were either the names did not match or there were different number of precincts across the elections, we attempted to locate additional information about the precincts from the counties themselves. Following this step, the precinct geometries were compared against the precinct assignments present in the 2022 L2 Alabama voter file dated 1/18/23. In some cases when modifying precinct geometries, modifications were made to prioritize the accuracy of the block-level disaggregated version of this file.

In order to create files where the placement of votes accurately reflects precincts split across districts, precinct geometries are split by the district geometries and the appropriate votes are assigned to the split positions of the precinct. When votes appear in a precinct for a particular contest but the area of the district for that contest does not intersect with the precinct at all, these votes are removed from the file. When creating the file at the State Legislative Lower District Level, a total of 160 votes were removed with 4 votes in District 32 removed in Calhoun-:-OXFORD CHEAHA CLUBHOUSE, 53 votes in District 68 removed in Clarke-:-COFFEVILLE HIGH, 2 votes in District 75 removed in Elmore-:-TRI-CITY BAPT_, 40 votes in District 92 removed in Escambia-:-FLOMATON COMM_ and 61 votes in District 73 removed in Shelby-:-CHELSEA COMM_ CTR_. For the purposes of disaggregation, users should use split files, but if a user is interested in analyzing all election data at once, the zip folder contains a "no_splits" file, in which precincts may contain votes for more than one district.
