...
def main() -> None:
    ...
    if pct.canonical_candidate_id.duplicated().any():
        raise ValueError("PCT candidate-cycle features are not unique by canonical_candidate_id")

    keep = ["canonical_candidate_id", "votesmart_candidate_id", "match_method",
            "pct_dimensions_scored", "pct_policies_scored", "pct_response_items_scored", *DIMENSIONS]
    candidate = candidates.merge(pct[keep], on="canonical_candidate_id", how="left",
                                 validate="one_to_one")
