...
function renderMap(){const d=DATA[active],map=$('#map'),tip=$('#tooltip');map.innerHTML='';map.setAttribute('aria-label',`${d.cycle} Alabama ${d.chamber} overperformance map`);d.paths.forEach(p=>{const x=d.winners[p.district],display=mapMetric(d,p.district),raw=mapRawValue(d,p.district),status=d.districtStatus[String(p.district)]||'No election record available',el=document.createElementNS('http://www.w3.org/2000/svg','path');el.setAttribute('d',p.path);el.setAttribute('fill',color(display));el.setAttribute('class','district'+(selected===p.district?' selected':''));el.setAttribute('tabindex','0');el.setAttribute('aria-label',x&&raw!=null?`District ${p.district}, ${raw>=0?'Democrat':'Republican'} overperformed by ${Math.abs(raw).toFixed(1)}, won by ${x.candidate}`:`District ${p.district}, ${status}`);el.onmouseenter=e=>{tip.style.display='block';tip.innerHTML=x&&raw!=null?`<b>District ${p.district}</b><br>${mapDescription()}<br>${raw>=0?'Democrat':'Republican'} +${Math.abs(raw).toFixed(1)} points<br>Won by ${esc(x.candidate)}`:`<b>District ${p.district}</b><br>${esc(x?'Selected benchmark unavailable':status)}`;moveTip(e)};el.onmousemove=moveTip;el.onmouseleave=()=>tip.style.display='none';el.onclick=()=>{selected=p.district;detail(x);renderMap()};el.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();el.onclick()}};map.appendChild(el)});$('#map-title').textContent=`${d.cycle} Alabama ${d.chamber[0].toUpperCase()+d.chamber.slice(1)} overperformance`;$('#vintage').textContent='Boundaries: '+d.mapVintage;$('#map-sub').textContent=mapDescription();$('#legend-ticks').innerHTML='<span>R +30</span><span>R +10</span><span>Even</span><span>D +10</span><span>D +30</span>'}
