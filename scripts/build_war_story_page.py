...
<section class="model-status"><div class="status-card feature"><span>Historical CMO architecture</span><b>Cycle-held-out Fundamentals+</b><p>Each candidate is compared with a regularized expectation trained without that election cycle. The expectation begins with the same-cycle statewide ticket in the district, then uses demographics, regional context, finance, incumbency, chamber, presidential context, and available candidate history for a limited adjustment.</p></div><div class="status-card"><b>__CYCLE_COUNT__</b><span>Historical cycles</span></div><div class="status-card"><b>__ELIGIBLE_RACES__</b><span>Eligible D vs. R races</span></div><div class="status-card"><b>3</b><span>CMO specifications</span></div></section>
...
<section class="method"><h2>How to read CMO</h2><p>Fundamentals+ begins with the same-cycle statewide ticket margin measured inside the legislative district, then adds a shrunk, four-point-capped adjustment using demographics, regional context where available, finance, incumbency, chamber, presidential context, and candidate-history indicators. Total CMO is the actual legislative margin minus that cycle-held-out expectation. Republican values are sign-reversed district residuals, making the index zero-sum within each race by construction.</p><p>The displayed band measures disagreement between cycle-held-out and ordinary cross-fitted scores; it is not a 95% confidence interval. Results apply only to contested Democratic-versus-Republican Alabama legislative races and should not be interpreted as a forecast or universal candidate-quality measure. Earlier cycles may use later cycles for training.</p><p><a href="cmo-methodology.html">Read the full CMO methodology</a>, <a href="index.html">view the 2026 forecast</a>, or read the <a href="methodology.html#models">forecast methodology</a>.</p><div class="source">Model output: <code>preliminary_cmo_candidates.csv</code>. Sources include Alabama election returns and campaign-finance records, Census ACS, presidential precinct returns, and enacted legislative district files.</div></section></main><div class="tooltip" id="tooltip"></div>
