...
<section class="model-status"><div class="status-card feature"><span>Historical CMO architecture</span><b>Cycle-held-out Fundamentals+</b><p>Each candidate is compared with a regularized expectation trained without that election cycle, using the presidential baseline, national environment, demographics, regional context, finance, incumbency, chamber, and available candidate history.</p></div><div class="status-card"><b>__CYCLE_COUNT__</b><span>Historical cycles</span></div><div class="status-card"><b>__ELIGIBLE_RACES__</b><span>Eligible D vs. R races</span></div><div class="status-card"><b>3</b><span>CMO specifications</span></div></section>
<section class="intro"><p>Winning a race is not the same thing as running a strong campaign. Candidate Margin Overperformance asks: <em>how much better or worse was a candidate's two-party margin than the model expected in that race?</em></p><p><strong>Positive CMO indicates outperformance; negative CMO indicates underperformance.</strong> Headline Total CMO now uses the cycle-held-out Fundamentals+ expectation. Scores are margin percentage points—not wins or an independently identified candidate effect.</p></section>
...
<section class="rankings"><h2>Candidate rankings</h2><div class="note">Total CMO is the headline. Spending- and fundraising-adjusted scores are secondary sensitivity specifications. Select any row for the full candidate profile.</div><div class="filters"><input id="candidate-search" type="search" placeholder="Search candidate or district"><select id="scope-filter"><option value="active">Selected cycle and chamber</option><option value="all">All cycles and chambers</option></select><select id="party-filter"><option value="all">All parties</option><option value="D">Democratic</option><option value="R">Republican</option></select><select id="outcome-filter"><option value="all">All candidates</option><option value="winner">Winners</option><option value="incumbent">Incumbents</option></select></div><div class="table-wrap"><table><thead><tr><th data-sort="cycle">Cycle</th><th data-sort="district">District</th><th data-sort="candidate">Candidate</th><th data-sort="war">Total CMO ↕</th><th data-sort="percentile">Percentile</th><th data-sort="adjusted">Resource-adjusted</th><th data-sort="fundraisingAdjusted">Fundraising-adjusted</th><th data-sort="specificationRange">Spec. range</th><th data-sort="cycleTopTicket">Top-ticket margin</th><th data-sort="expectedMargin">Expected margin</th><th data-sort="margin">Candidate margin</th><th data-sort="votes">Votes</th></tr></thead><tbody id="rows"></tbody></table></div></section>
__VALIDATION_PANEL__
__ATTRIBUTION_PANEL__
<section class="downloads"><h2>Data and provenance</h2><p>Build updated August 16, 2026 from the canonical historical pipeline. Download the published rows and the diagnostics behind this page. When reusing results, credit the underlying source named above and identify CMO values as calculations by Jackson Hannan.</p><div class="download-links"><a href="data/preliminary_cmo_candidates.csv">Candidate output</a><a href="data/preliminary_cmo_races.csv">Race output</a><a href="data/cmo_diagnostics.csv">Cross-validation</a><a href="data/cmo_forward_validation.csv">Forward validation</a><a href="data/cmo_model_card.md">Model card</a><a href="cmo-methodology.html">Methodology</a></div></section>
<section class="method"><h2>How to read CMO</h2><p>Fundamentals+ begins with the prior presidential district margin and historically supported national-environment transfer, then adds a shrunk, four-point-capped adjustment using demographics, regional context where available, finance, incumbency, chamber, and candidate-history indicators. Total CMO is the actual legislative margin minus that cycle-held-out expectation. Republican values are sign-reversed district residuals, making the index zero-sum within each race by construction.</p><p>The displayed band measures disagreement between cycle-held-out and ordinary cross-fitted scores; it is not a 95% confidence interval. Results apply only to contested Democratic-versus-Republican Alabama legislative races and should not be interpreted as a forecast or universal candidate-quality measure. Earlier cycles may use later cycles for training, and 21 races use an explicitly flagged same-cycle statewide fallback where prior-presidential district geography is unavailable.</p><p><a href="cmo-methodology.html">Read the full CMO methodology</a>, <a href="index.html">view the 2026 forecast</a>, or read the <a href="methodology.html#models">forecast methodology</a>.</p><div class="source">Model output: <code>preliminary_cmo_candidates.csv</code>. Sources include Alabama election returns and campaign-finance records, Census ACS, presidential precinct returns, and enacted legislative district files.</div></section></main><div class="tooltip" id="tooltip"></div>
...
function detail(x){const box=$('#detail');if(!x){box.innerHTML='<div class="detail-empty">Select a district or candidate row to inspect the race.</div>';return}const width=x.high-x.low,stable=width<=15,quality=x.quality==='standard source checks passed',tier=x.modelTier==='sensitivity_1994'?'1994 sensitivity':'Core historical',history=allCandidates().filter(c=>c.personId&&c.personId===x.personId).sort((a,b)=>a.cycle-b.cycle),historyHtml=history.length>1?`<div class="decomp"><div class="decomp-title">Candidate history</div>${history.map(c=>`<div class="stat"><span>${c.cycle} ${c.chamber} ${c.district}</span><b>${fmt(c.war)}</b></div>`).join('')}</div>`:'';box.innerHTML=`${raceBox(x)}<h3>${esc(x.candidate)} <span class="tier-badge ${x.modelTier==='sensitivity_1994'?'sensitivity':''}">${tier}</span></h3><div class="party ${x.party}">${x.party==='D'?'Democratic':'Republican'} • District ${x.district}${x.incumbent?' • Incumbent':''}</div><div class="badges"><span class="badge">${x.percentile>=90?'Exceptional':x.percentile>=70?'Above average':x.percentile<=10?'Exceptional underperformance':'Typical range'}</span><span class="badge ${stable?'':'warn'}">${stable?'More stable':'High sensitivity'}</span><span class="badge ${x.signConsistent?'':'warn'}">${x.signConsistent?'Specifications agree':'Direction changes by specification'}</span><span class="badge ${quality?'':'warn'}">${quality?'Standard sources':'Source caution'}</span></div><div class="war-number">${fmt(x.war)}</div><div class="war-label">CMO points • ${x.percentile.toFixed(0)}th percentile</div><div class="distribution"><i style="left:${x.percentile}%"></i><div class="distribution-label"><span>Lowest</span><span>Median</span><span>Highest</span></div></div><div class="stat"><span>Cross-cycle stability band</span><b>${fmt(x.low)} to ${fmt(x.high)}</b></div><div class="stat"><span>Resource-adjusted CMO</span><b>${fmt(x.adjusted)}</b></div><div class="stat"><span>Fundraising-adjusted CMO</span><b>${fmt(x.fundraisingAdjusted)}</b></div><div class="stat"><span>Specification range</span><b>${x.specificationRange.toFixed(1)} points</b></div><div class="decomp"><div class="decomp-title">Margin decomposition</div><div class="stat"><span>Model-expected candidate margin</span><b>${fmt(x.expectedMargin)}</b></div><div class="stat"><span>Candidate's actual margin</span><b>${fmt(x.margin)}</b></div><div class="stat"><span>Difference (CMO)</span><b>${fmt(x.war)}</b></div></div><div class="decomp"><div class="decomp-title">Model context (descriptive, not causal contributions)</div><div class="context-grid"><div><span>Same-cycle ticket</span><b>${fmt(x.cycleTopTicket)}</b></div><div><span>Prior president (${x.priorPresYear?Math.round(x.priorPresYear):'year unavailable'})</span><b>${fmtMaybe(x.priorPres)}</b></div><div><span>Prior presidential trend</span><b>${x.presTrendAvailable?fmtMaybe(x.priorPresSwing):'Unavailable'}</b></div><div><span>Incumbency</span><b>${x.incumbent?'Candidate incumbent':'Candidate not coded incumbent'}</b></div><div><span>Nonwhite share</span><b>${pct(x.nonwhiteShare)}</b></div><div><span>White college share</span><b>${pct(x.whiteCollegeShare)}</b></div></div></div><div class="decomp"><div class="decomp-title">Source quality</div><div class="quality-grid"><div><span>Statewide allocation</span><b>${esc(x.baselineMethod||'Unavailable')}</b></div><div><span>Baseline fallback</span><b>${pct(x.baselineFallbackShare)}</b></div><div><span>Presidential source</span><b>${x.priorPresComplete?'Complete':'Incomplete'} · fallback ${pct(x.priorPresFallbackShare)}</b></div><div><span>Demographics</span><b>${esc(x.demographicsMethod||'Unavailable')}${x.demographicReferenceYear?' · '+Math.round(x.demographicReferenceYear):''}</b></div><div><span>Incumbency evidence</span><b>${x.incumbencyConflict?'Conflict neutralized':x.incumbencyComplete?'Complete':'Incomplete'}</b></div><div><span>Campaign finance</span><b>Spending ${x.financeComplete?'complete':'incomplete'} · FTM ${x.ftmFinanceComplete?'complete':'incomplete'}</b></div><div><span>Readiness</span><b>${esc(x.readinessStatus||tier)}</b></div><div><span>Votes</span><b>${x.votes.toLocaleString()}</b></div></div></div>${historyHtml}<div class="explain">${x.war>=0?'This candidate ran ahead of':'This candidate ran behind'} the cross-fitted model expectation by about <b>${Math.abs(x.war).toFixed(1)} points</b>.<br><br><b>Data note:</b> ${esc(x.quality)}</div>`}
...
function renderRows(){const d=DATA[active],scope=$('#scope-filter').value,q=$('#candidate-search').value.toLowerCase(),party=$('#party-filter').value,outcome=$('#outcome-filter').value,source=scope==='all'?allCandidates():d.candidates.map(x=>({...x,section:active,cycle:d.cycle,chamber:d.chamber})),rows=source.filter(x=>(party==='all'||x.party===party)&&(outcome==='all'||(outcome==='winner'&&x.winner)||(outcome==='incumbent'&&x.incumbent))&&(!q||x.candidate.toLowerCase().includes(q)||String(x.district)===q||String(x.cycle)===q||`${x.chamber} ${x.district}`.includes(q))).sort((a,b)=>{let A=a[sortKey],B=b[sortKey];return(typeof A==='string'?A.localeCompare(B):A-B)*sortDir});$('#rows').innerHTML=rows.map(x=>`<tr tabindex="0" data-section="${x.section}" data-district="${x.district}" data-party="${x.party}"><td>${x.cycle} ${x.chamber==='house'?'H':'S'}</td><td>${x.district}</td><td class="cand"><i class="party-dot ${x.party}"></i>${esc(x.candidate)}${x.winner?' <small>✓</small>':''}${x.modelTier==='sensitivity_1994'?' <span class="tier-badge sensitivity">Sensitivity</span>':''}</td><td class="num"><b>${fmt(x.war)}</b></td><td class="num">${x.percentile.toFixed(0)}th</td><td class="num">${fmt(x.adjusted)}</td><td class="num">${fmt(x.fundraisingAdjusted)}</td><td class="num">${x.specificationRange.toFixed(1)} ${x.signConsistent?'✓':'⚠'}</td><td class="num">${fmt(x.cycleTopTicket)}</td><td class="num">${fmt(x.expectedMargin)}</td><td class="num">${fmt(x.margin)}</td><td class="num">${x.votes.toLocaleString()}</td></tr>`).join('');document.querySelectorAll('#rows tr').forEach(row=>{row.onclick=()=>selectCandidate(row.dataset.section,row.dataset.district,row.dataset.party);row.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();row.onclick()}}})}
...
<section id="baseline"><h2>3. The expected baseline</h2><p>The headline Fundamentals+ expectation begins with prior presidential district performance and the historically supported national-environment transfer. It then applies 20% of a ridge adjustment, capped at four points, using demographics, regional context where available, finance and its availability, incumbency and open-seat status, chamber, and available prior-candidate indicators. Twenty-one races without prior-presidential district geography use the same-cycle statewide index as a labeled fallback.</p><div class="callout"><b>Why a regularized model?</b> Ridge regression and the 20% shrinkage limit unstable adjustments in a small, correlated dataset. The richer model improves average historical error but does not beat the Basic forecast in 2022.</div></section>
...
if __name__ == "__main__":
    ...
    SITE_METHODOLOGY_OUTPUT.write_text(build_methodology_page(eligible_races), encoding="utf-8")
    ...
    for filename in ("preliminary_cmo_races.csv", "cmo_diagnostics.csv", "cmo_benchmark_diagnostics.csv",
                     "cmo_forward_validation.csv", "cmo_forward_interval_calibration.csv"):
        shutil.copy2(WAR / filename, site_data / filename)
