...
<section id="performance"><div class="section-head"><div class="kicker">Bloc comparison</div><h2>Within-election performance differences</h2><p>Each estimate is the traditionalist-populist minus progressive-modern difference after comparing candidates within the same election year and chamber. Election performance was not used to assign candidates to a bloc.</p></div><div class="panel"><div id="headline" class="headline-comparison" aria-live="polite"></div><div class="method headline-method"><b>Comparison set:</b> only cycle-and-chamber cells containing both blocs. Lines show approximate 95% intervals with repeat appearances clustered by candidate identity. These are descriptive adjusted contrasts, not causal effects.</div></div></section>
...
function renderHeadline(){const rows=DATA.headlineBlocPerformance,all=rows.flatMap(r=>[r.ci_low,r.ci_high]).filter(Number.isFinite),limit=Math.max(10,Math.ceil(Math.max(...all.map(Math.abs))/5)*5),left=v=>5+90*(v+limit)/(2*limit),axis=`<div class="headline-axis"><span></span><div><span>${fmt(-limit)}</span><span>0</span><span>${fmt(limit)}</span></div><span>Traditionalist minus progressive</span></div>`;$('#headline').innerHTML=axis+rows.map(r=>`<div class="headline-row"><h3>${esc(OUTCOMES[r.outcome])}</h3><div class="headline-track" role="img" aria-label="${esc(OUTCOMES[r.outcome])}: traditionalist minus progressive ${fmt(r.difference)}, approximate 95 percent interval ${fmt(r.ci_low)} to ${fmt(r.ci_high)}"><i class="headline-ci" style="left:${left(r.ci_low)}%;width:${left(r.ci_high)-left(r.ci_low)}%;background:${COLORS[TRAD]}"></i><i class="headline-point" style="left:${left(r.difference)}%;background:${COLORS[TRAD]}"></i></div><div class="headline-values"><b>${fmt(r.difference)}</b><span>95% interval ${fmt(r.ci_low)} to ${fmt(r.ci_high)}</span><small>n=${r.n}; ${r.strata} cycle/chamber cells</small></div></div>`).join('')}
...
def build() -> str:
    ...
    supplemental_css = (
        ...
        ".headline-track{background:#f5f1f2}"
        ...
        "@media(max-width:780px){.headline-axis{display:none}.headline-method{margin:0 12px 15px}}"
