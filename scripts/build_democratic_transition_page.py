...
def payload() -> dict:
    ...
    headline = []
    for outcome in ("candidate_quality_index", "candidate_federal_overperformance",
...
<section id="performance"><div class="section-head"><div class="kicker">Bloc comparison</div><h2>Performance relative to three expectations</h2><p>All three comparisons are shown on the same visual structure. Election performance was not used to assign candidates to a bloc.</p></div><div class="panel"><div id="headline" class="headline-comparison" aria-live="polite"></div><div class="key" style="padding:0 20px 12px"><span><i class="trad"></i>Traditionalist-populist</span><span><i class="prog"></i>Progressive-modern</span></div></div></section>
...
<section id="time"><div class="section-head"><div class="kicker">CQI and absolute ideology by era</div><h2>CQI association by era</h2><p>Each row estimates the change in Candidate Quality Index associated with a one-standard-deviation move toward the conservative end of the nationally comparable Shor–McCarty scale. The common axis makes differences in estimate and uncertainty directly comparable.</p></div><div id="eraEvidence" class="era-forest panel"></div></section>
...
function renderEra(){const outcome='candidate_quality_index',eras=['pre_2008','2008_2014','post_2016'];$('#eraEvidence').innerHTML=eras.map(era=>{const r=DATA.era.find(x=>x.sample===`D:${era}`&&x.outcome===outcome);const available=r&&r.coefficient!=null;return `<div class="era-card"><b>${available?fmt(r.coefficient):'Not estimated'}</b><span>${eraLabel(era)} · CQI per SD rightward</span><p>${available?`95% CI ${fmt(r.ci_low)} to ${fmt(r.ci_high)}; n=${r.n}`:`Only ${r?.n||0} observed Democratic candidate-cycles with absolute ideology; insufficient for this specification.`}</p></div>`}).join('')}
...
function renderRows(){const q=$('#candidateSearch').value.toLowerCase(),rows=DATA.cluster.members.filter(x=>x.party==='D'&&(!q||`${x.canonical_name} ${x.chamber} ${x.district}`.toLowerCase().includes(q))).sort((a,b)=>a.cycle-b.cycle||a.canonical_name.localeCompare(b.canonical_name));$('#candidateRows').innerHTML=rows.map(d=>`<tr data-id="${esc(d.canonical_candidate_id)}"><td>${esc(d.canonical_name)}</td><td>${d.cycle} ${String(d.chamber).toUpperCase()}-${d.district}</td><td>${d.cluster_label===TRAD?'Traditionalist-populist':'Progressive-modern'}</td><td class="num">${fmt(d.candidate_quality_index)}</td><td class="num">${fmt(d.candidate_federal_overperformance)}</td><td class="num">${fmt(d.candidate_presidential_overperformance)}</td></tr>`).join('');document.querySelectorAll('#candidateRows tr').forEach(tr=>tr.onclick=()=>selectCandidate(DATA.cluster.members.find(x=>x.canonical_candidate_id===tr.dataset.id)))}
function renderHeadline(){const rows=DATA.headlineBlocPerformance,all=rows.flatMap(r=>[r.traditionalist_mean-1.96*r.traditionalist_se,r.traditionalist_mean+1.96*r.traditionalist_se,r.progressive_mean-1.96*r.progressive_se,r.progressive_mean+1.96*r.progressive_se]).filter(Number.isFinite),limit=Math.max(10,Math.ceil(Math.max(...all.map(Math.abs))/5)*5),left=v=>5+90*(v+limit)/(2*limit);$('#headline').innerHTML=rows.map(r=>{const tlo=r.traditionalist_mean-1.96*r.traditionalist_se,thi=r.traditionalist_mean+1.96*r.traditionalist_se,plo=r.progressive_mean-1.96*r.progressive_se,phi=r.progressive_mean+1.96*r.progressive_se;return `<div class="headline-row"><h3>${esc(OUTCOMES[r.outcome])}</h3><div class="headline-track" aria-label="Traditionalist ${fmt(r.traditionalist_mean)} and progressive ${fmt(r.progressive_mean)}"><i class="headline-ci" style="left:${left(tlo)}%;width:${left(thi)-left(tlo)}%;background:${COLORS[TRAD]}"></i><i class="headline-point" style="left:${left(r.traditionalist_mean)}%;background:${COLORS[TRAD]}"></i><i class="headline-ci" style="left:${left(plo)}%;width:${left(phi)-left(plo)}%;top:22px;background:${COLORS[PROG]}"></i><i class="headline-point" style="left:${left(r.progressive_mean)}%;top:17px;background:${COLORS[PROG]}"></i></div><div class="headline-values"><span><b>${fmt(r.traditionalist_mean)}</b>Traditionalist n=${r.traditionalist_n}</span><span><b>${fmt(r.progressive_mean)}</b>Progressive n=${r.progressive_n}</span><span><b>${fmt(r.difference)}</b>Difference</span></div></div>`}).join('')}
function renderTransition(){const cycles=[...new Set(DATA.democraticTransition.map(x=>x.cycle))].sort((a,b)=>a-b),thin=[];$('#transitionChart').innerHTML=`<div class="transition-axis"><span></span><div><span>0%</span><span>50%</span><span>100%</span></div><span>sample</span></div>`+cycles.map(c=>{const rows=DATA.democraticTransition.filter(x=>x.cycle===c),total=rows.reduce((s,x)=>s+x.n,0);if(total<10)thin.push(`${c} (n=${total})`);return `<div class="transition-row ${total<10?'thin':''}"><strong>${c}</strong><div class="stack">${[TRAD,PROG].map(label=>{const r=rows.find(x=>x.cluster_label===label),n=r?.n||0;return `<button style="width:${total?100*n/total:0}%;background:${COLORS[label]}" aria-label="${label}, ${n} of ${total}" title="${label}: ${n} of ${total}"></button>`}).join('')}</div><small>n=${total}</small></div>`}).join('')+(thin.length?`<p class="sample-note">Low-coverage cycles: ${thin.join(', ')}. Their shares are descriptive and should not be read as stable caucus estimates.</p>`:'')}
function renderEra(){const outcome='candidate_quality_index',eras=['pre_2008','2008_2014','post_2016'],rows=eras.map(era=>({era,r:DATA.era.find(x=>x.sample===`D:${era}`&&x.outcome===outcome)})),bounds=rows.flatMap(x=>[x.r?.ci_low,x.r?.ci_high]).filter(Number.isFinite),limit=Math.max(5,Math.ceil(Math.max(...bounds.map(Math.abs),1)/2)*2),left=v=>5+90*(v+limit)/(2*limit);$('#eraEvidence').innerHTML=rows.map(({era,r})=>{const available=r&&r.coefficient!=null;return `<div class="forest-row"><b>${eraLabel(era)}</b><div class="forest-track">${available?`<i class="forest-ci" style="left:${left(r.ci_low)}%;width:${left(r.ci_high)-left(r.ci_low)}%"></i><i class="forest-point" style="left:${left(r.coefficient)}%"></i>`:''}</div><div class="forest-value">${available?`<b>${fmt(r.coefficient)} CQI per SD</b>95% CI ${fmt(r.ci_low)} to ${fmt(r.ci_high)}; n=${r.n}`:`<b>Not estimated</b>Only ${r?.n||0} observed candidate-cycles.`}</div></div>`}).join('')}
...
initIssues();renderHeadline();renderTransition();renderProfile();renderDistribution();renderEra();renderIssue();renderCases();renderConstellation();renderRows();labelInteractivePoints();enableCandidateRows();
...
def build() -> str:
    """Add small-screen containment rules to the generated analysis page."""
    page = _build_unresponsive()
    mobile_distribution_css = (
    ...
    return page.replace("</head>", mobile_distribution_css + "</head>", 1)
