...
OUTPUT = ROOT / "Alabama 2026 Legislative Forecast.html"
SITE = ROOT / "docs"
MAPS = {
    "house": ROOT / "Results and Shapefiles" / "tl_2025_01_sldl" / "tl_2025_01_sldl.shp",
    "senate": ROOT / "Results and Shapefiles" / "tl_2025_01_sldu" / "tl_2025_01_sldu.shp",
}
...
def build_payload():
    ...
    payload={"meta":{"pollAsOf":poll_date.isoformat(),"buildDate":build_date.isoformat(),
                     ...
                     "model":"poll_adjusted_direct_baseline","version":"2026.08.15-baseline-first"}}
...
<section class="section method"><div><h2>How to read this forecast</h2><p>The headline starts with each district’s observed 2024 presidential result and adds a district-specific demographic environment change derived from generic-ballot polling, Catalist history, YouGov cross-tabs, and Alabama ACS composition.</p><p>Incumbency, demographic-residual, finance, and prior-CMO layers were evaluated only as residual adjustments. None passed the rule requiring improvement in both average and latest-cycle forward MAE, so none changes the headline. The direct baseline’s mean forward MAE is 12.6 points.</p><div class="method-links"><a href="FORECAST_METHODOLOGY.md">Full methodology</a><a href="data/processed/war/2026_forecast_decomposition.csv">District data</a><a href="data/processed/war/2026_residual_layer_backtest_summary.csv">Backtests</a></div></div><div class="caveat"><b>Experimental forecast.</b><p>Only two comparable forward cycles support the current uncertainty estimates. Probabilities use 50,000 simulations with shared statewide and chamber error plus district-specific error. Single-major-party districts are treated as fixed seats for chamber summaries even when an independent is present. Gray, dashed districts are unresolved or unmodeled rather than toss-ups.</p><p>Polling staleness is displayed at the top of the page. The environment component should be rebuilt when new polling becomes available.</p></div></section></main><footer class="site-footer"><div><b>Model and analysis by Jackson Hannan</b><span>Alabama 2026 Legislative Forecast</span></div><nav aria-label="Jackson Hannan profiles"><a href="https://github.com/JacksonAHannan" target="_blank" rel="me noopener">GitHub</a><a href="https://www.instagram.com/topsoilintraining/" target="_blank" rel="me noopener">Instagram</a><a href="https://substack.com/@jacksonhannan" target="_blank" rel="me noopener">Substack</a><a href="https://www.linkedin.com/in/jackson-hannan" target="_blank" rel="me noopener">LinkedIn</a></nav></footer><div class="tooltip" id="tooltip" role="tooltip"></div><script>const DATA=__PAYLOAD__;__JS__</script></body></html>"""


UNCERTAINTY_CAVEAT = """<div class="caveat"><b>Experimental uncertainty estimates.</b><p>District probabilities and seat ranges are calculated from 50,000 simulations around the poll-adjusted baseline. Each simulation includes an Alabama-wide error shared by every district, an additional House or Senate error shared within that chamber, and district-specific error.</p><p>The error scales are estimated from forward tests of the 2018 and 2022 elections. The historical archive does not provide a comparable 2008 district presidential baseline for testing 2010, while 2014 serves as the initial training cycle. With only two forward elections, the estimated correlations and probabilities should be treated as provisional rather than fully calibrated.</p><p>Districts with only one major-party nominee are treated as fixed Democratic or Republican seats in chamber summaries. Independent-only and otherwise unresolved districts remain unmodeled and are displayed in gray.</p><p>The national-environment projection uses polling through the date shown above and should be refreshed as new polling becomes available.</p></div>"""
...
<section id="candidate"><h2>4. Candidate and finance layers</h2><p>Incumbency, demographic residuals, finance, and prior candidate margin overperformance were tested only as adjustments to the strong baseline. Finance is descriptive and potentially endogenous: strong candidates can raise more money, so it is not interpreted as a clean causal effect.</p><p>Prior CMO uses only historical out-of-fold scores, exact normalized candidate-and-party matches, and shrinkage toward zero. Too few repeat candidates exist to promote this layer prospectively, so it remains a scenario.</p></section>
<section id="validation"><h2>5. Forward validation and promotion</h2><p>Models train only on elections earlier than the cycle being predicted. A layer is promoted only when it improves both average expanding-window forward MAE and the latest-cycle MAE.</p><table class="method-table"><thead><tr><th>Specification</th><th>Mean forward MAE</th><th>Latest MAE</th><th>Promoted</th></tr></thead><tbody>__BACKTEST_ROWS__</tbody></table><p>The historical archive lacks an equivalent 2008 district presidential baseline for 2010, so comparable forward tests are limited. The headline should therefore be understood as a disciplined baseline forecast, not a richly trained candidate-quality model.</p></section>
...
<section id="limitations"><h2>7. Important limitations</h2><ul><li>Only two comparable forward cycles support the current uncertainty estimates.</li><li>National race and education crosstabs are not joint race-by-education samples; the model combines them with ACS and historical estimates.</li><li>Alabama polling is sparse, so national movements are transferred through demographic structure rather than measured directly in the state.</li><li>Polling house effects and mode effects are not yet estimated explicitly.</li><li>Candidate, incumbency, finance, and CMO scenarios have not cleared the declared promotion gate.</li><li>Probabilities are provisional and should not be read as fully calibrated long-run frequencies.</li></ul></section>
...
def main():
    ...
    page=page.replace('<a href="FORECAST_METHODOLOGY.md">Full methodology</a>',
