...
<section id="validation"><h2>5. Forward validation and promotion</h2><p>Models train only on elections earlier than the cycle being predicted. Testing is limited to the post-2008 period, with holdouts in 2014, 2018, and 2022. A layer is promoted only when it improves both average expanding-window forward MAE and the latest-cycle MAE.</p><table class="method-table"><thead><tr><th>Specification</th><th>Mean forward MAE</th><th>Latest MAE</th><th>Promoted</th></tr></thead><tbody>__BACKTEST_ROWS__</tbody></table><p>The national environment adjustment improves 2022 MAE from 12.03 to 9.88 points, but worsens the three-cycle mean from 17.34 to 17.93 because it performs poorly in 2014. The demographic-response variant is weaker still. Finance likewise improves the latest holdout but not average error, and the era-weighted federal scenario fails the gate. All remain auditable scenarios; the headline stays anchored to the poll-adjusted 2024 presidential result.</p></section>
