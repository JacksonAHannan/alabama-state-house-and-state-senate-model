...
UNCERTAINTY_CAVEAT = """<div class="caveat"><b>Experimental uncertainty estimates.</b><p>District probabilities and seat ranges are calculated from 50,000 simulations around the poll-adjusted baseline. Each simulation includes an Alabama-wide error shared by every district, an additional House or Senate error shared within that chamber, and district-specific error.</p><p>The error scales are estimated from post-2008 expanding-window forward tests with holdouts in 2014, 2018, and 2022. With only three forward elections, the estimated correlations and probabilities should be treated as provisional rather than fully calibrated.</p><p>Districts with only one major-party nominee are treated as fixed Democratic or Republican seats in chamber summaries. Independent-only and otherwise unresolved districts remain unmodeled and are displayed in gray.</p><p>The national-environment projection uses polling through the date shown above and should be refreshed as new polling becomes available.</p></div>"""
...
<section id="candidate"><h2>4. Candidate, finance, and federal-realignment layers</h2><p>Incumbency, demographic residuals, finance, and prior candidate margin overperformance were tested only as adjustments to the strong baseline. The current finance scenario uses Alabama state campaign-finance receipts through August 14, 2026; unmatched filings remain unknown rather than being treated as zero. Finance is descriptive and potentially endogenous: strong candidates can raise more money, so it is not interpreted as a clean causal effect.</p><p>A separate federal-realignment scenario learns legislative residuals from same-cycle U.S. House and U.S. Senate results. Validation begins after the 2008 nationalization break, and post-2016 observations receive twice the weight of 2010-2014 observations. The prospective anchor is the allocated 2024 presidential result, so the scenario does not invent an unavailable 2026 state-office result.</p><p>Prior CMO uses only historical out-of-fold scores, exact normalized candidate-and-party matches, and shrinkage toward zero. Too few repeat candidates exist to promote this layer prospectively, so it remains a scenario.</p></section>
<section id="validation"><h2>5. Forward validation and promotion</h2><p>Models train only on elections earlier than the cycle being predicted. Testing is limited to the post-2008 period, with holdouts in 2014, 2018, and 2022. A layer is promoted only when it improves both average expanding-window forward MAE and the latest-cycle MAE.</p><table class="method-table"><thead><tr><th>Specification</th><th>Mean forward MAE</th><th>Latest MAE</th><th>Promoted</th></tr></thead><tbody>__BACKTEST_ROWS__</tbody></table><p>The finance scenario improves the latest holdout but not average error. The era-weighted federal scenario also fails the declared promotion gate. Both remain auditable scenarios; the headline stays anchored to the poll-adjusted 2024 presidential result.</p></section>
...
def main():
    ...
    page=page.replace(
        ...
        "Incumbency, demographic-residual, current fundraising, federal-realignment, and prior-CMO layers were evaluated only as residual adjustments. None passed the rule requiring improvement in both average and latest-cycle forward MAE, so none changes the headline. The direct baseline’s post-2008 mean forward MAE is 17.3 points.")
