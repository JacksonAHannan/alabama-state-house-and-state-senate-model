...
def build_payload():
    ...
    model_copy={
        "headline":("Headline forecast","The generic ballot shifts each district's 2024 presidential margin, then the post-2016 WAR model adds its generic structural expectation. Candidate-specific WAR is fixed at zero."),
    ...
    payload={"meta":{"pollAsOf":poll_date.isoformat(),"buildDate":build_date.isoformat(),
             ...
             "models":[],"contributionVariables":["Generic-ballot district baseline","Generic WAR structure","WAR incumbency effect","Candidate WAR (fixed at zero)","Polling-error scenario"],"provenance":[
                 ...
                 {"category":"Methodology","source":"Generic-ballot environment and generic-candidate WAR structural forecast definitions","asOf":build_date.isoformat(),"download":"data/alabama_war_forecast_v1_manifest.json"}
             ]}
...
<details class="quick-method"><summary>Forecast views</summary><ol><li><b>Headline</b> applies the polling-implied national swing and the generic WAR structural expected gap; candidate-specific WAR remains zero.</li><li><b>Dem scenario</b> and <b>Rep scenario</b> move every district by one historical national polling-error standard deviation.</li><li>District probabilities use a Student-t curve; chamber summaries use 50,000 simulations with shared statewide and chamber uncertainty.</li></ol></details></section>
...
def build_methodology_v3(css: str, payload: dict) -> str:
    ...
    holdout_assessment = (
        "improved on" if structural.mae < baseline.mae else "performed worse than"
    )
    scale = float(manifest["probability"]["scale"])
    styles = css + (ASSETS / "methodology.css").read_text(encoding="utf-8")
    return f'''<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Forecast methodology · Alabama 2026</title><style>{styles}</style></head><body>
...
<main class="methodology-shell"><header class="methodology-hero"><div class="kicker">Model documentation</div><h1>Forecast methodology</h1><p class="dek">A generic-candidate WAR forecast anchored to the national generic ballot, with prior candidate performance explicitly excluded.</p><div class="status-row"><span class="status-chip">Headline <b>generic ballot + WAR structure</b></span><span class="status-chip">Polling through <b>{payload['meta']['pollAsOf']}</b></span><span class="status-chip">Build <b>{manifest['build_id']}</b></span></div></header>
...
<section id="identity"><h2>1. Forecast identity</h2><div class="formula">2026 Democratic margin = 2024 district presidential margin + national generic-ballot swing + generic WAR structure + WAR incumbency effect + candidate-specific WAR</div><p>The selected post-2016 Southern WAR model supplies the structural expected gap, including its symmetric incumbency effect. Candidate-specific residual WAR is fixed at zero, so no candidate's past strength enters the forecast.</p></section>
