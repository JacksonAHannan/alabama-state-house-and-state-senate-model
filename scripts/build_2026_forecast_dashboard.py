...
<section id="validation"><h2>5. Forward validation and promotion</h2><p>Models train only on elections earlier than the cycle being predicted. Testing is limited to the post-2008 period, with holdouts in 2014, 2018, and 2022. A layer is promoted only when it improves both average expanding-window forward MAE and the latest-cycle MAE.</p><table class="method-table"><thead><tr><th>Specification</th><th>Mean forward MAE</th><th>Latest MAE</th><th>Promoted</th></tr></thead><tbody>__BACKTEST_ROWS__</tbody></table><p>Applying the full national swing in every cycle improves 2022 but performs poorly in 2014. A post-2016 ramp—zero weight through 2014, half weight in 2018, and full weight in 2022 and 2026—reduces mean cycle-level MAE from 17.34 to 16.42 and 2022 MAE from 12.03 to 9.88. The demographic-response, finance, and era-weighted federal variants fail the gate.</p><div class="callout"><b>Exploratory structural-break rule.</b>The ramp is motivated by the predeclared 2016 realignment hypothesis but was evaluated on the same three holdouts used to choose it. It preserves the existing 2026 poll-adjusted margin numerically and should be re-tested unchanged after the next comparable election.</div></section>
...
def main():
    ...
    page=page.replace(
        ...
        "Incumbency, demographic-residual, current fundraising, federal-realignment, and prior-CMO layers were evaluated only as residual adjustments. The post-2016 national-environment ramp passed the declared comparison and formalizes the existing poll-adjusted headline; other layers remain scenarios. Its post-2008 mean forward MAE is 16.4 points.")
