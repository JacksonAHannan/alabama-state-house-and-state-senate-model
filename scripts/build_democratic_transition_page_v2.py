...
def representative_cases(members: pd.DataFrame) -> pd.DataFrame:
    ...
    for group in GROUPS:
        sample = democrats[
            ...
            & democrats.candidate_quality_index.notna()
            & democrats.candidate_federal_overperformance.notna()
        ].copy()
        if sample.empty:
            continue
        targets = [
            ("Near group median WAR", sample.candidate_quality_index.median(), "candidate_quality_index"),
        ...
        for kind, target, field in targets:
            ...
            rows.append({
                ...
                "candidate_quality_index": float(row.candidate_quality_index),
...
def payload() -> dict:
    ...
    quality = pd.read_csv(
        CURRENT_QUALITY, usecols=["canonical_candidate_id", "candidate_quality_index"]
    )
    members = (members.drop(columns=["candidate_cmo", "candidate_quality_index",
                                     "candidate_quality_residual"], errors="ignore")
               .merge(quality, on="canonical_candidate_id", how="left", validate="one_to_one"))
    if members.candidate_quality_index.isna().any():
        raise ValueError("Every clustered candidate-cycle must have current WAR")
    ...
    era = pd.DataFrame(ideology["era"])
    era = era[
        era["sample"].astype(str).str.startswith("D:")
        & era.outcome.eq("candidate_quality_index")
        & era.term.eq("absolute_conservatism_z")
    ].copy()
...
</style></head><body><header><div class="mast"><div><div class="brand">Jackson Hannan</div><div class="tag">Alabama legislative elections</div></div><nav class="nav" aria-label="Site navigation"><a href="index.html">Forecast</a><a href="cmo.html">CMO</a><a href="ideology-performance.html" aria-current="page">Ideology &amp; caucuses</a><a href="cmo-methodology.html">Methodology</a></nav></div></header><main class="shell"><div class="hero"><div class="kicker">Historical Democratic ideology and performance</div><h1>Alabama Democratic groupings, 1998–2022</h1><p class="dek">The current issue-position evidence identifies three Democratic regions: traditionalist-populist, bridge-coalition, and progressive-modern. The headline result is WAR—Wins Above Replacement—followed by raw performance against federal and presidential baselines.</p><div class="finding"><b>Headline result:</b> after holding election cycle and chamber constant, traditionalist-populist and bridge-coalition Democrats both record higher WAR than progressive-modern Democrats in the available common contexts. Raw ticket comparisons are presented separately because they do not isolate candidate quality.</div></div><nav class="contents" aria-label="On this page"><a href="#performance">WAR</a><a href="#overview">Groups</a><a href="#transition">Transition</a><a href="#positions">Positions</a><a href="#distribution">Distribution</a><a href="#time">Cycles</a><a href="#issues">Issues</a><a href="#cases">Candidates</a><a href="#candidate-explorer">Similarity map</a><a href="#continuous">Continuous ideology</a><a href="#methods">Methods</a></nav>

<section id="performance"><div class="section-head"><div class="kicker">Headline result</div><h2>WAR relative to progressive-modern candidates</h2><p>WAR means Wins Above Replacement. It estimates candidate quality in margin points after accounting for the electoral context in which a candidate ran. Each comparison below holds cycle and chamber constant and uses only contexts containing both groups.</p></div><div id="warHeadline" class="war-headline"></div><div class="panel"><div id="contrastList" class="contrast-list"></div></div></section>
...
<section id="methods"><div class="section-head"><div class="kicker">Coverage and interpretation</div><h2>Data and method</h2></div><div id="sourceGrid" class="source-grid"></div><div class="method"><h3>WAR naming and interpretation</h3><p>WAR means Wins Above Replacement. The name and candidate-quality framing are adapted from <a href="https://split-ticket.org/2025/08/15/deconstructing-war/">Split Ticket's WAR methodology</a>. This Alabama implementation is expressed in margin points relative to a replacement-level expectation; it is not a literal count of seats or wins.</p></div><div class="method"><h3>Group construction</h3><p id="clusterMethod"></p></div><div class="method"><h3>Performance attachment</h3><p>Election performance was not used to create the groups. WAR, incumbency, fundraising, demographics, district partisanship, and era are also attached only after group assignment. Adjusted comparisons use cycle-and-chamber fixed effects and person-clustered uncertainty.</p></div><div class="method"><h3>Limits</h3><p>The clustering silhouette is modest, so the groups should be read as regions on a continuum. Evidence is more available for officeholders and is not missing at random. Raw averages combine changing eras and district contexts. The analysis is descriptive and does not prove that a particular issue caused electoral performance.</p></div></section></main><div id="tip" class="tip"></div><footer>Research and model by Jackson Hannan · <a href="cmo.html">CMO</a> · <a href="cmo-methodology.html">Methodology</a></footer>
...
const OUTCOME_LABELS={candidate_quality_index:'WAR',candidate_federal_overperformance:'Raw vs. federal baseline',candidate_presidential_overperformance:'Raw vs. previous president'};
...
function renderGroups(){const by=(g,o)=>DATA.groupSummary.find(x=>x.group===g&&x.outcome===o);$('#groupGrid').innerHTML=GROUPS.map(g=>{const war=by(g,'candidate_quality_index'),fed=by(g,'candidate_federal_overperformance'),pres=by(g,'candidate_presidential_overperformance');return `<article class="group-card ${cardClass(g)}"><h3>${SHORT[g]}</h3><div class="count">${war.n} candidate-cycles · ${war.people} people</div><div class="metric-grid"><div><b>${fmt(war.mean)}</b><span>Mean WAR</span></div><div><b>${fmt(fed.mean)}</b><span>Mean vs. federal</span></div><div><b>${fmt(pres.mean)}</b><span>Mean vs. president</span></div></div></article>`}).join('')}
...
function renderWarHeadline(){const rows=DATA.contrasts.filter(r=>r.outcome==='candidate_quality_index');$('#warHeadline').innerHTML=rows.map(r=>`<article class="war-card ${cardClass(r.group)}"><div class="war-number">${fmt(r.difference)}</div><h3>${esc(SHORT[r.group])} WAR</h3><p>Compared with progressive-modern Democrats in ${r.strata} common cycle/chamber contexts. 95% interval ${fmt(r.ci_low)} to ${fmt(r.ci_high)}; n=${r.n}.</p></article>`).join('')}
function renderContrasts(){const war=DATA.contrasts.filter(r=>r.outcome==='candidate_quality_index'),tickets=DATA.contrasts.filter(r=>r.outcome!=='candidate_quality_index');$('#contrastList').innerHTML=contrastMarkup(war);$('#ticketContrastList').innerHTML=contrastMarkup(tickets)}
